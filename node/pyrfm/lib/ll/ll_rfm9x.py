...
class LinkLayer(Prototype):
	...
	def sendStr(self,text):
		...
		#now_time = list(array.array('B',(dt.encode('iso8859-1'))))
		message = text + "*" + dt
		data=list(array.array('B',(message.encode('iso8859-1'))))
		return self.send(data)
		
		
	def sendRelay(self,e):
		if self.waitPacketSent():
			self.setOpModeIdle()

			self.PL.writeRegister(RFM_REG_FIFO_ADDR_PTR, 0)
			self.PL.writeRegister(RFM_REG_FIFO, e)
	...
	def send(self,data):
		...
		if self.waitPacketSent():
			...
			#The headers
			#<to> <from> <id> <flags> #可自行修改
			self.PL.writeRegister(RFM_REG_FIFO, [1,0,self._TX_id,0])
			#The message data
